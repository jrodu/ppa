...
Here is a score you can try (note- don't forget to name the function so that you can access it later!):
...
Here are a transformation you can try (note- don't forget to name the function so that you can access it later!):
```{r ppaapp_transformation, eval=FALSE}
panel_data %>% mutate(y=y^2)
```
Go ahead, try a few!  Try to chain them together in different ways and navigate through the various transformations to get a feel for using the navigation tree.

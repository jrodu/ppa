...
newFilterFunctionUI <- function(id) {
  ...
  shiny::tagList(
    shiny::actionButton(ns("newvaluefilterfunctionbutton"),
  ...
  shiny::moduleServer(id, function(input, output, session){
    ...
    shiny::observeEvent(input$newvaluefilterfunctionbutton, {
      ...
      shinyalert::shinyalert(
        ...
        textInput(ns("functionPlacerName"), label=NULL,
        ...
        placeholder=
          ...
          Your output should be: a single numeric score or a tibble with one
          row and one column
        ...
        shiny::actionButton(ns("addfilterfunction"), "add function to list")
                             ))
    ...
    shiny::observeEvent(input$addfilterfunction, {
      ...
      pipeline_variables$filter_value_functions <-
        pipeline_variables$filter_value_functions %>%
        dplyr::add_row(name=fn_label, fn=input$placerval)

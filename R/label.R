...
labelUI <- function(id) {
  ...
  #   shiny::tags$p("To label selected panels, choose label from list, or
  # create a
  #                 new label (you must click on the label to create it).
  # Then, click
  #                 the 'Apply Label' button.  Click 'Save labels' to save
  # labels and
  ...
  #   shiny::selectizeInput(ns("filterbylabel"), "Select by Label",
...
labelServer <- function(id, pipeline_variables) {
  shiny::moduleServer(id, function(input, output, session){
    ...
    shiny::tagList(
      shiny::tags$h4("Label panels"),
      shiny::tags$p("To label selected panels, choose label from list, or create
                  a new label by typing in the selection box.  Then, click
                  the 'Apply Label' button.
                    To select by label, select the label(s) of interest from the
                    'Select by Label' dropdown menu."),
      shiny::selectizeInput(ns('label'), 'Label', choices=character(),
    ...
    shiny::observeEvent(input$applylabel, {
      # get unselected states (but not hidden) and make them 0,
      # then when left_join the hidden ones will be NA.
      if(isTruthy(input$label)){
        ...
        shiny::updateSelectizeInput(session, "filterbylabel",
                                    selected=character(),
    ...
    shiny::observeEvent(input$filterbylabel, {
      totselect <- NULL
      # get unselected states (but not hidden) and make them 0, then when
      # left_join the hidden ones will be NA.
      select_this <- pipeline_variables$labels %>%
        dplyr::mutate(totselect = rowSums(
          dplyr::across(tidyselect::all_of(input$filterbylabel)), na.rm=TRUE
          )) %>%

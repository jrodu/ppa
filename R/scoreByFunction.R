...
scoreByFunctionServer <- function(id, pipeline_variables) {
  shiny::moduleServer(id, function(input, output, session){
    ...
    output$scorebyfunction <- shiny::renderUI(
      ...
      shiny::tagList(
        ...
        shiny::tags$p("Create a new function by clicking 'create new function'.
                  ...
                  To select panels by the score function, select the function from the list,
                  and click 'Select by Score'.  After you click this button, an empirical CDF (ecdf) plot of panel score
                  values will appear, as well as a horizontal line indicating a panel selection threshold.
        ...
        newFilterFunctionUI(ns("fbyv")),
        filterbuttonUI(ns("filterselectize")), shiny::actionButton(ns("edit_filter"), "Edit"),
    ...
    shiny::observeEvent(input$filterValue, {
      ...
      #compute ecdf needs:
      buffer <- ggplot2::resolution(pipeline_variables$filtereddf$score, zero=FALSE)*3
    ...
    shiny::observeEvent(input$edit_filter, {
      ...
      shinyalert::shinyalert(html = TRUE,
                             ...
                             showConfirmButton = FALSE,text = tagList(
                               shinyjs::disabled(textInput(ns("editFilterName"), label=NULL, value=filter_fn())),
                               textAreaInput(ns("editfilterval"),
    ...
    shiny::observeEvent(input$submitedfilterfunction, {
      ...
      pipeline_variables$filter_value_functions <- pipeline_variables$filter_value_functions %>% dplyr::mutate(fn=base::replace(.data$fn, .data$name==fn_label, input$editfilterval))

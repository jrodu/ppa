...
transformUI <- function(id) {
  ...
  #  placeholder="Select a previously defined transform function"),
  #                  multiple=FALSE),
  #   shiny::br(),
  #
  ...
  shiny::moduleServer(id, function(input, output, session){
    ...
    output$transform <- shiny::renderUI(
      shiny::tagList(
        ...
        shiny::tags$p(
        "This functionality allows you to do quick transformations to examine
        different aspects of your panel data.  Transformations can utilize any
        variables in your data set, though you should", shiny::strong("modify"),
        "the variables used in the plot, and not create new variables. This is
        really meant to examine (potentially conditional) transformations of
        the plotted variables. After you click 'transform data' a tree will
        appear that contains the transformations that you have applied.
        To return to a previous state, click on the node related to that state.
        Any further transformations will appear as a new branch
        off of that node.  Finally, once you have used a function to do a
        transformation, you should not edit that function.  If you would
        like to use a similar function, create a new one.
                      "),
        newTransformFunctionUI(ns("tform")),
#        shiny::selectizeInput(ns('trnsfm'), label=NULL, choices=character(),
#                              options = list(create = TRUE,
...
#                              multiple=FALSE),
    ...
    transform_fn <- transformbuttonServer("transformselectize",
                                          pipeline_variables)
    ...
    shiny::observeEvent(input$transform, {
      ...
      if(!isTruthy(transform_fn())) {#check if selectize is null
        ...
        fn_txt_tmp <- pipeline_variables$transform_functions %>%
          dplyr::filter(.data$name==transform_fn()) %>% dplyr::select(.data$fn)
        fn_snippet <- fn_txt_tmp$fn
        fn_txt <- paste('f_transform <- function(panel_data) {',
                        fn_txt_tmp$fn, '}', sep='')
      ...
      if(is.null(errorhandle)){
        ...
        tryCatch(
          tmp_try <- transform_data(pipeline_variables$df_main, f_transform),
          error = function(e) {
      ...
      if(is.null(errorhandle)){
        ...
        tmpcheck <- pipeline_variables$tree %>%
          dplyr::filter(.data$name==transform_fn(),
                        .data$parentId==pipeline_variables$current_anchor,
                        .data$type=="transform")
        if(nrow(tmpcheck)==0){
          max_tree <- max(pipeline_variables$tree$id)
          pipeline_variables$tree <- pipeline_variables$tree %>%
            dplyr::add_row(id = max_tree+1,
                           parentId = pipeline_variables$current_anchor,
                           name=transform_fn(), type="transform",
                           expression1=fn_snippet)
      ...
      gargoyle::trigger("send_to_panel_plot")
        ...
        shinyalert::shinyalert(
          paste0("Oops!",
                 ...
                 errorhandle,
                 ". Select the function and click 'edit' to edit the
                 function."),
          type = "error")
      }
      }else{
        shinyalert::shinyalert(
          paste0("Oops!",
                 ...
                 Maybe this is a hint: ",
                 errorhandle,
                 ". Select the function and click 'edit' to edit
                 the function."),
          type = "error")
      }
      }
    ...
    shiny::observeEvent(input$edit_tform, {
      ...
      shinyalert::shinyalert(
        ...
        text = tagList(
          shinyjs::disabled(textInput(ns("editTransformName"),
                                      label=NULL, value=transform_fn())),
    ...
    shiny::observeEvent(input$submitedtransformfunction, {
      ...
      pipeline_variables$transform_functions <-
        pipeline_variables$transform_functions %>%
        dplyr::mutate(
          fn=base::replace(.data$fn, .data$name==fn_label,
                           input$edittransformval))
    ...
    #   shiny::updateSelectizeInput(session, inputId = "trnsfm",
    ...
    shiny::observe({
      ...
      for(i in seq_along(ancestors)){
        if(ancestors[i]==1){
          ...
          #print(ancestors[i])
          #print(pipeline_variables$tree)
          fn_txt_tmp <- pipeline_variables$tree %>%
            dplyr::filter(id==ancestors[i]) %>% dplyr::select(.data$expression1)
          fn_txt <- paste('f_transform <- function(panel_data) {
                          ', fn_txt_tmp$expression1, '}', sep='')
          eval(parse(text = fn_txt))

          pipeline_variables$df_main <-
            transform_data(pipeline_variables$df_main, f_transform)
        }
      }
       pipeline_variables$shift_data()
       pipeline_variables$trigger_newdat()
       ...
       session$sendCustomMessage(
         type = "data",
         message = list(
           lines = jsonlite::toJSON(pipeline_variables$newdat),
